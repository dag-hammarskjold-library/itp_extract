...
has 'delete_field' => (
	...
	code => sub {
		...
		for my $field ($self->get_fields) {
			delete $self->{fields}->{$field->tag} if refaddr($delete) == refaddr($field);
		}
	}
);
has 'change_tag' => (
...
sub to_tie_ixhash {
	...
	my $record = Tie::IxHash->new;
	
	$record->Push(_id => 0 + $self->id);
	$record->Push(leader => $self->leader);
	
	my @cf;
	for my $field (grep {! $_->is_leader && $_->is_controlfield} $self->fields) {
		my $cf = Tie::IxHash->new (
			tag => $field->tag,
			value => $field->text
		);
		push @cf, $cf;
	}
	$record->Push(controlfield => \@cf);
	
	my @df;
	for my $field (grep {$_->is_datafield} $self->fields) {
		my $df = Tie::IxHash->new (
			tag => $field->tag,
			ind1 => $field->ind1,
			ind2 => $field->ind2,
			subfield => [
	...
	$record->Push(datafield => \@df);
	
	return $record;
}

sub to_tie_ixhash_alt {
	my $self = shift;
...
sub to_mongo_alt {
	...
	return $col->replace_one({_id => 0 + $self->id}, $self->to_tie_ixhash_alt, {upsert => 1});
}

sub from_mongo {
	my ($self,$href) = @_;
	
	$self->add_field(MARC::Field->new(tag => '000', text => $href->{leader}));
	
	for my $cf ( $href->{controlfield}->@* ) {
		my $nf = MARC::Field->new(tag => $cf->{tag}, text => $cf->{value});
		$self->add_field($nf);
	}
	for my $df ( sort {$a->{tag} cmp $b->{tag} } $href->{datafield}->@* ) {
		my $nf = MARC::Field->new(tag => $df->{tag}, indicators => $df->{ind1}.$df->{ind2});
		
		for my $sub ( $df->{subfield}->@* ) {
			$nf->set_sub($sub->{code},$sub->{value});
		}
		$self->add_field($nf);
	}
